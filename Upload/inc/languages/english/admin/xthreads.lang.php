...
$l['threadfields_name_desc'] = 'The key through which this field is accessed through.  Should be kept to alphanumeric characters, underscores (_) and hypens (-) only.  Note that only inputs for this field on newthread/editpost are automatically added for you; forumdisplay/showthread etc templates aren\'t affected, so you will need to make changes to the relevant templates for this to be useful.  Use <code>{$threadfields[\'<em style="color: #00A000;">key</em>\']}</code> in templates to reference this field (this is slightly different for file inputs - more info given below if you choose to make a file input).';
$l['threadfields_file_name_info'] = 'Variables are referenced with <code>{$threadfields[\'<em style="color: #00A000;">key</em>\'][\'<em style="color: #0000A0;">item</em>\']}</code>, where <em style="color: #0000A0;">item</em> can be one of the following:
<ul style="margin-top: 0.2em;">
	...
	<li><em>dims</em> - an array containing width/height of uploaded image if the option to require image uploads is chosen.  For example <code>{$threadfields[\'myimage\'][\'dims\'][\'w\']}</code> would get the width of the uploaded image.</li>
	<li><em>thumbs</em> - an array containing width/height of thumbnails (if used).  For example <code>{$threadfields[\'myimage\'][\'thumbs\'][\'320x240\'][\'w\']}</code> would get the real image width of the 320x240 thumbnail.</li>
</ul>';
...
$l['threadfields_blankval_desc'] = 'You can specify a custom value to be displayed if the user leaves this field blank.  This field is not parsed, so you can enter HTML etc here.';
...
$l['threadfields_formatmap_desc'] = 'A list of formatting definitions.  Separate items with newlines, and input/output pairs with the 3 characters, <em>{|}</em>.  The format map will translate defined inputs to the defined outputs.  For example, if you specify, for this field, <em>Resolved{|}&lt;span style=&quot;color: green;&quot;&gt;Resolved&lt;/span&gt;</em>, then if the user enters/selects &quot;Resolved&quot; for this field, it will be outputted in green wherever {$threadfields[...]} is used.';
